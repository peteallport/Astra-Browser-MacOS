...
| Service access | Historical fresh Wikipedia and IANA runs verified Browser Run and `gpt-6-astra` through the provider-native gateway route. New operators must configure and verify their own gateway access; the app has no built-in deployed endpoint or credential alias. |
